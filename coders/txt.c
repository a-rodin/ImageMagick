  ...
  *colorspace='\0';
  ...
  while (ReadBlobString(image,text) != (char *) NULL)
  {
    ...
    if (q == (PixelPacket *) NULL)
      continue;
    q->red=ScaleAnyToQuantum(pixel.red,image->depth,scale);
    q->green=ScaleAnyToQuantum(pixel.green,image->depth,scale);
    q->blue=ScaleAnyToQuantum(pixel.blue,image->depth,scale);
    if (image->colorspace == CMYKColorspace)
      {
        indexes=GetAuthenticIndexQueue(image);
        *indexes=ScaleAnyToQuantum(pixel.index,image->depth,scale);
      }
    if (image->matte != MagickFalse)
