...
static inline double GetFITSQuantum(Image *image,int bits_per_pixel)
{
  switch (image->depth >> 3)
  {
    ...
    case 2:
      return((double) ReadBlobShort(image));
    case 4:
    {
      if (bits_per_pixel > 0)
        return((double) ReadBlobLong(image));
      return((double) ReadBlobFloat(image));
    }
    case 8:
    {
      if (bits_per_pixel > 0)
        return((double) ReadBlobLongLong(image));
    }
    default:
      break;
  }
...
static void GetFITSQuantumExtrema(Image *image,const int bits_per_pixel,
  double *minima,double *maxima)
{
  ...
  *minima=GetFITSQuantum(image,bits_per_pixel);
  *maxima=(*minima);
  for (i=1; i < (MagickOffsetType) number_pixels; i++)
  {
    quantum=GetFITSQuantum(image,bits_per_pixel);
    if (quantum < *minima)
      *minima=quantum;
    if (quantum > *maxima)
      *maxima=quantum;
...
static inline double GetFITSQuantumRange(const unsigned long depth)
{
  return((double) ((MagickOffsetType) GetQuantumRange(depth)));
}

static void SetFITSUnsignedQuantum(const size_t length,
  ...
  */
  ...
  for (scene=0; scene < (long) fits_info.number_planes; scene++)
  {
    ...
    if ((fits_info.min_data != 0.0) || (fits_info.max_data != 0.0))
      {
        if ((fits_info.bits_per_pixel != 0) && (fits_info.max_data == 0.0))
          fits_info.max_data=GetFITSQuantumRange((unsigned long)
            fits_info.bits_per_pixel);
      }
    else
      GetFITSQuantumExtrema(image,fits_info.bits_per_pixel,&fits_info.min_data,
        &fits_info.max_data);
    /*
    ...
    */
    ...
    quantum_info->scale=QuantumRange*fits_info.scale;
    quantum_info->minimum=fits_info.min_data;
    quantum_info->maximum=fits_info.max_data;
  ...
  Image *image)
{
  ...
  */
  ...
  (void) ResetMagickMemory(fits_info,' ',2880UL*sizeof(*fits_info));
  /*
    Initialize image header.
  */
  ...
  offset+=80;
  (void) FormatMagickString(header,MaxTextExtent,"BITPIX  =           %10ld",
  ...
  (void) FormatMagickString(header,MaxTextExtent,"BZERO   =         %E",
    image->depth > 8 ? GetFITSQuantumRange(image->depth) : 0.0);
  (void) strncpy(fits_info+offset,header,strlen(header));
  offset+=80;
  ...
  if (image->endian == LSBEndian)
    {
      (void) FormatMagickString(header,MaxTextExtent,"XENDIAN = 'SMALL'");
      (void) strncpy(fits_info+offset,header,strlen(header));
      offset+=80;
    }
  ...
  */
  ...
  quantum_info=DestroyQuantumInfo(quantum_info);
  length=(size_t) (2880-TellBlob(image) % 2880);
  if (length != 0)
    {
