...
#define BeginXMLCode  "begin_xml_code"
#define EndXMLCode  "end_xml_code"
  ...
  */
  ...
  for (c=ReadBlobByte(image); c != EOF; c=ReadBlobByte(image))
  {
    ...
    */
    ...
    if (LocaleNCompare(BeginXMLCode,command,strlen(BeginXMLCode)) == 0)
      {
        /*
          Read XML code.
        */
        ...
        for (i=0; c != EOF; i++)
        {
          ...
          *p='\0';
          p=command;
          if (LocaleNCompare(EndXMLCode,command,strlen(EndXMLCode)) == 0)
            break;
        }
        (void) SetImageProfile(image,"xml-code",profile);
        profile=DestroyStringInfo(profile);
        continue;
      }
    /*
    ...
    if (page == 1)
      {
        ...
        */
        ...
        profile=GetImageProfile(image,"xml-code");
        if (profile != (StringInfo *) NULL)
          {
            /*
              Embed XML profile.
