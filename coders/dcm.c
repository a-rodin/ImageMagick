  ...
  if (image->depth != (1UL*MAGICKCORE_QUANTUM_DEPTH))
    {
      ...
      */
      ...
      quantum_scale=GetQuantumScale(image->depth);
      for (i=0; i < (long) (GetQuantumRange(image->depth)+1); i++)
        scale[i]=ScaleAnyToQuantum((unsigned long) i,image->depth,
          quantum_scale);
    }
        ...
        for (y=0; y < (long) image->rows; y++)
        {
          ...
          for (x=0; x < (long) image->columns; x++)
          {
            if (samples_per_pixel == 1)
              {
                if (bytes_per_pixel == 1)
                  index=polarity != MagickFalse ? max_value-
                    (unsigned long) ReadBlobByte(image) :
                    (unsigned long) ReadBlobByte(image);
                else
                  if ((bits_allocated != 12) || (significant_bits != 12))
                    {
                      if (msb_first != MagickFalse)
                        index=polarity != MagickFalse ? max_value-
                          (unsigned long) ReadBlobMSBShort(image) :
                          (unsigned long) ReadBlobMSBShort(image);
                      else
                        index=polarity != MagickFalse ? max_value-
                          (unsigned long) ReadBlobLSBShort(image) :
                          (unsigned long) ReadBlobLSBShort(image);
                      if (signed_data == 1)
                        index-=32767;
                    }
