      ...
      if (*option == '-')
        {
          if (LocaleCompare("compress",option+1) == 0)
            ...
            {
              imageInfo->depth=MAGICKCORE_QUANTUM_DEPTH;
              arg++;
...
if (0)
  ...
  status=WriteImage ( imageInfo, original );
  if ( status == MagickFalse )
    {
      (void) printf ( "Failed to write file in format %s\n", imageInfo->magick )
;
      (void) fflush(stdout);
      exit_status = 1;
      goto program_exit;
  ...
  original=DestroyImageList(original);
...
;
  ...
  fuzz_factor = 0.07;
  if ( !strcmp( "PCD", format ) || !strcmp( "PCDS", format ) )
    fuzz_factor = 0.20;
#endif
