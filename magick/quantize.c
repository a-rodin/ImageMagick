...
static MagickBooleanType
  RiemersmaDither(Image *,ViewInfo *,CubeInfo *,const unsigned int);

static void Riemersma(Image *image,ViewInfo *image_view,CubeInfo *cube_info,
...
static MagickBooleanType RiemersmaDither(Image *image,ViewInfo *image_view,
...
static MagickBooleanType DitherImage(Image *image,CubeInfo *cube_info)
{
  ...
  */
  ...
  for (depth=1; i != 0; depth++)
    i>>=1;
  if ((1L << depth) < MagickMax((long) image->columns,(long) image->rows))
    depth++;
  cube_info->offset=0;
  ...
  const unsigned long levels,const MagickBooleanType dither)
{
  ...
  */
  ...
  exception=(&image->exception);
  q=QueueCacheViewAuthenticPixels(posterize_view,0,0,posterize_image->columns,
    1,exception);
  if (q == (PixelPacket *) NULL)
    {
