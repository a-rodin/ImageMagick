  ...
  if ( resample_filter->limit_reached ) {
    switch ( resample_filter->virtual_pixel ) {
      /*  This has already been handled above
        ...
        case MaskVirtualPixelMethod:
        case MaskVirtualPixelMethod:
          status=InterpolateResampleFilter(resample_filter->image,
            resample_filter->view,IntegerInterpolatePixel,u0,v0,
            &pixel);
          break;
      */
      case UndefinedVirtualPixelMethod:
...
%  mapping.  They do not need to be the orthogonal derivatives, but can be
%  caclated using radial/angluar derivatives.
%
...
%         The order is the order you generally would figure out the
...
#define F_UNITY 4.0
  ...
  */
  { double alpha, beta, gamma, Major, Minor;
    ...
    This actually used as part of parallelogram bounds
