    ...
    case DePolarDistortion:
    {
      ...
      */
      ...
      if ( number_arguments >= 4 ) {
        ...
        coeff[2] = (double)(image->columns-1)/2.0+image->page.x;
        coeff[3] = (double)(image->rows-1)/2.0+image->page.y;
      }
      ...
      if ( number_arguments == 6 ) {
        ...
        if ( fabs(coeff[4]-coeff[5]) < MagickEpsilon )
          coeff[5] += Magick2PI; /* angle 0 to 0 is a full circle */
      }
      else {
        coeff[4] = -MagickPI;
        coeff[5] = MagickPI;
...
{ \
  ...
  max.y = MagickMax(max.y,p.y); \
    ...
    switch (method)
    {
      ...
      {
        min.x = min.y = 0;
        max.y = coeff[0]-coeff[1];
        max.x = (coeff[0]+coeff[1])*MagickPI;
    ...
    switch (method) {
      ...
      case DePolarDistortion:
      {
        ...
        fprintf(stderr, "  -fx 'xx=sin((i+page.x)*%lg %+lg) %+lg;\n",
                         coeff[6], -coeff[4], coeff[2] );
        fprintf(stderr, "       jj=cos((j+page.y)*%lg %+lg) %+lg;\n",
                         coeff[7], -coeff[1], coeff[3] );
        fprintf(stderr, "       v.p{xx,yy}'\n");
        break;
      }
      case BarrelDistortion:
      ...
      { double xc,yc;
        ...
        fprintf(stderr, "       ii=ii%s(%lg*rr*rr*rr %+lg*rr*rr %+lg*rr %+lg);\n",
             method == BarrelDistortion ? "*" : "/",
             coeff[0],coeff[1],coeff[2],coeff[3]);
        fprintf(stderr, "       jj=jj%s%lg*rr*rr*rr %+lg*rr*rr %+lg*rr %+lg;\n",
             method == BarrelDistortion ? "*" : "/",
             coeff[4],coeff[5],coeff[6],coeff[7]);
        ...
        switch (method)
        {
          ...
          { /* Rect/Cartesain/Cylinder to Polar View */
            ...
            */
            ...
            s.x = s.x*coeff[6] + (double)image->columns/2.0
                            + image->page.x;
            s.y = (s.y - coeff[1])*coeff[7] + image->page.y+0.5;
            break;
          }
          case DePolarDistortion:
          { /* Polar to Cylindical  */
...
#define SparseColorTag  "Distort/SparseColor"
  ...
  */
  coeff = GenerateCoefficients(image, (DistortImageMethod *)&method,
       number_arguments, arguments, number_colors, exception);
  if ( coeff == (double *) NULL )
    return((Image *) NULL);
