    ...
    for (x=0; x < (long) blur_image->columns; x++)
    {
      ...
      if (((channel & OpacityChannel) == 0) || (image->matte == MagickFalse))
      ...
      else
        {
          ...
          for (i=0; i < (long) width; i++)
          {
            alpha=(MagickRealType) (QuantumScale*(QuantumRange-(p+i)->opacity));
            pixel.red+=(*k)*alpha*(p+i)->red;
            pixel.green+=(*k)*alpha*(p+i)->green;
            pixel.blue+=(*k)*alpha*(p+i)->blue;
            gamma+=(*k)*alpha;
            k++;
          }
          gamma=1.0/(fabs((double) gamma) <= MagickEpsilon ? 1.0 : gamma);
...
static Quantum **DestroyPixelThreadSet(Quantum **pixels)
{
  ...
  return((Quantum **) RelinquishMagickMemory(pixels));
}
...
#define DespeckleImageTag  "Despeckle/Image"
  ...
  */
  ...
  buffer=AcquirePixelThreadSet(length);
  if ((pixels == (Quantum **) NULL) || (buffer == (Quantum **) NULL))
    {
      if (buffer != (Quantum **) NULL)
        buffer=DestroyPixelThreadSet(buffer);
      if (pixels != (Quantum **) NULL)
  ...
  */
  ...
  for (layer=0; layer <= 3; layer++)
  {
    ...
    (void) ResetMagickMemory(pixels[id],0,length*sizeof(**pixels));
    ...
    for (y=0; y < (long) image->rows; y++)
    {
      ...
      for (x=0; x < (long) image->columns; x++)
      {
        switch (layer)
        {
          case 0: pixels[id][j]=p->red; break;
          case 1: pixels[id][j]=p->green; break;
          case 2: pixels[id][j]=p->blue; break;
          case 3: pixels[id][j]=p->opacity; break;
          default: break;
        }
        p++;
        j++;
    ...
    (void) ResetMagickMemory(buffer[id],0,length*sizeof(**buffer));
    for (i=0; i < 4; i++)
    {
      Hull(X[i],Y[i],image->columns,image->rows,pixels[id],buffer[id],1);
      Hull(-X[i],-Y[i],image->columns,image->rows,pixels[id],buffer[id],1);
      Hull(-X[i],-Y[i],image->columns,image->rows,pixels[id],buffer[id],-1);
      Hull(X[i],Y[i],image->columns,image->rows,pixels[id],buffer[id],-1);
    }
    ...
    for (y=0; y < (long) image->rows; y++)
    {
      ...
      for (x=0; x < (long) image->columns; x++)
      {
        switch (layer)
        {
          case 0: q->red=pixels[id][j]; break;
          case 1: q->green=pixels[id][j]; break;
          case 2: q->blue=pixels[id][j]; break;
          case 3: q->opacity=pixels[id][j]; break;
          default: break;
        }
        q++;
        j++;
  ...
  #pragma omp critical
#endif
        proceed=SetImageProgress(image,DespeckleImageTag,layer,3);
        if (proceed == MagickFalse)
          status=MagickFalse;
      }
  }
...
static MedianPixelList *DestroyMedianPixelList(MedianPixelList *pixel_list)
{
  ...
  return((MedianPixelList *) RelinquishMagickMemory(pixel_list));
}
  ...
  for (y=0; y < (long) spread_image->rows; y++)
  {
    ...
    for (x=0; x < (long) spread_image->columns; x++)
    {
      (void) ResamplePixelColor(resample_filter[id],(double) x+width*
        (GetPseudoRandomValue()-0.5),(double) y+width*
        (GetPseudoRandomValue()-0.5),&pixel);
