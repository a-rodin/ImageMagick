  ...
  NexusInfo *nexus_info,ExceptionInfo *exception)
{
  ...
  */
  ...
  if (image->clip_mask == (Image *) NULL)
    return(MagickFalse);
  cache_info=GetImagePixelCache(image,exception);
...
static MagickBooleanType CloneDiskToDiskPixelCache(CacheInfo *cache_info,
  CacheInfo *source_info,ExceptionInfo *exception)
{
  MagickOffsetType
    count,
  ...
  if (OpenPixelCacheOnDisk(source_info,IOMode) == MagickFalse)
    {
      ThrowFileException(exception,FileOpenError,"UnableToOpenFile",
  ...
  if ((cache_info->active_index_channel != MagickFalse) &&
      (source_info->active_index_channel != MagickFalse))
    {
      register IndexPacket
        *indexes;
      ...
      */
      ...
      for (y=0; y < (long) rows; y++)
      {
        ...
        count=ReadPixelCacheRegion(source_info,source_info->offset+
          source_offset,length,(unsigned char *) indexes);
        if ((MagickSizeType) count != length)
          break;
        offset-=cache_info->columns*sizeof(*indexes);
        count=WritePixelCacheRegion(cache_info,cache_info->offset+offset,length,
  ...
  */
  ...
  for (y=0; y < (long) rows; y++)
  {
    ...
    count=ReadPixelCacheRegion(source_info,source_info->offset+source_offset,
...
static MagickBooleanType CloneDiskToMemoryPixelCache(CacheInfo *cache_info,
  CacheInfo *source_info,ExceptionInfo *exception)
{
  MagickOffsetType
    count,
      ...
      (source_info->active_index_channel != MagickFalse))
    {
      ...
      */
      ...
      for (y=0; y < (long) rows; y++)
      {
        ...
        count=ReadPixelCacheRegion(source_info,source_info->offset+offset,
          length,(unsigned char *) indexes);
        if ((MagickSizeType) count != length)
          break;
  ...
  */
  ...
  for (y=0; y < (long) rows; y++)
  {
    ...
    count=ReadPixelCacheRegion(source_info,source_info->offset+offset,length,
      (unsigned char *) pixels);
    if ((MagickSizeType) count != length)
      break;
    q-=cache_info->columns;
...
static MagickBooleanType CloneMemoryToDiskPixelCache(CacheInfo *cache_info,
  CacheInfo *source_info,ExceptionInfo *exception)
{
  MagickOffsetType
    count,
...
static MagickBooleanType CloneMemoryToMemoryPixelCache(CacheInfo *cache_info,
  ...
  CacheInfo *source_info,ExceptionInfo *exception)
{
  if ((cache_info->type != DiskCache) && (source_info->type != DiskCache))
    return(CloneMemoryToMemoryPixelCache(cache_info,source_info,exception));
  if ((cache_info->type == DiskCache) && (source_info->type == DiskCache))
    return(CloneDiskToDiskPixelCache(cache_info,source_info,exception));
  if (source_info->type == DiskCache)
    return(CloneDiskToMemoryPixelCache(cache_info,source_info,exception));
  return(CloneMemoryToDiskPixelCache(cache_info,source_info,exception));
}

/*
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
static void DestroyPixelCache(Image *image)
{
  ...
  if (image->cache == (void *) NULL)
    return;
  image->cache=DestroyPixelCacheInfo(image->cache);
}

/*
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%                                                                             %
%                                                                             %
+   D e s t r o y P i x e l C a c h e I n f o                                 %
%                                                                             %
%                                                                             %
%                                                                             %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  DestroyPixelCacheInfo() deallocates memory associated with the pixel cache.
%
%  The format of the DestroyPixelCacheInfo() method is:
%
%      Cache DestroyPixelCacheInfo(Cache cache)
%
%  A description of each parameter follows:
%
%    o cache: the pixel cache.
...
MagickExport Cache DestroyPixelCacheInfo(Cache cache)
{
  CacheInfo
    *cache_info;
  ...
  const unsigned long number_threads)
{
  ...
  return((NexusInfo **) RelinquishMagickMemory(nexus_info));
}

/*
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
%      Cache GetImagePixelCache(Image *image,ExceptionInfo *exception)
%
%  A description of each parameter follows:
%
%    o image: the image.
...
MagickExport Cache GetImagePixelCache(Image *image,ExceptionInfo *exception)
{
  CacheInfo
    *cache_info;
          ...
          if (status != MagickFalse)
            {
              ...
              if (status != MagickFalse)
                {
                  status=ClonePixelCache(clone_info,cache_info,exception);
                  if (status != MagickFalse)
                    {
                      cache_info->reference_count--;
                      image->cache=clone_image.cache;
                      (void) SetPixelCacheVirtualMethod(image,
                        cache_info->virtual_pixel_method);
                    }
                }
            }
        }
  ...
  ExceptionInfo *exception)
{
  ...
  */
  ...
  region.height=rows;
  pixels=SetPixelCacheNexusPixels(image,&region,nexus_info);
  if (pixels == (PixelPacket *) NULL)
    return((const PixelPacket *) NULL);
  ...
  ExceptionInfo *exception)
{
  ...
  *offset+=cache_info->length+pagesize-(cache_info->length % pagesize);
  clone_info=(CacheInfo *) DestroyPixelCacheInfo(clone_info);
  return(status);
}

/*
  ...
  NexusInfo *nexus_info,ExceptionInfo *exception)
{
  ...
  */
  ...
  region.width=columns;
  region.height=rows;
  return(SetPixelCacheNexusPixels(image,&region,nexus_info));
}

/*
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      */
      ...
      for (y=0; y < (long) rows; y++)
      {
        count=WritePixelCacheRegion(cache_info,cache_info->offset+number_pixels*
          sizeof(PixelPacket)+offset*sizeof(*p),length,(unsigned char *) p);
        if ((MagickSizeType) count < length)
          break;
        p+=nexus_info->region.width;
        offset+=cache_info->columns;
  ...
  switch (cache_info->type)
  {
    ...
    case DiskCache:
    {
      ...
      */
      ...
      {
        count=WritePixelCacheRegion(cache_info,cache_info->offset+offset*
          sizeof(*p),length,(unsigned char *) p);
        if ((MagickSizeType) count < length)
          break;
        p+=nexus_info->region.width;
        offset+=cache_info->columns;
