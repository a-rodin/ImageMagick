...
%
%      MagickBooleanType AcquireOneCacheViewPixel(const ViewInfo *cache_view,
...
%
%      PixelPacket *GetCacheView(ViewInfo *cache_view,const long x,
%        const long y,const unsigned long columns,const unsigned long rows)
%
%  A description of each parameter follows:
%
%    o cache_view: the address of a structure of type ViewInfo.
%
%    o x,y,columns,rows:  These values define the perimeter of a region of
%      pixels.
%
*/
MagickExport PixelPacket *GetCacheView(ViewInfo *cache_view,const long x,
  const long y,const unsigned long columns,const unsigned long rows)
{
  PixelPacket
    *pixels;
...
%
%      IndexPacket *GetCacheViewIndexes(ViewInfo *cache_view)
%
%  A description of each parameter follows:
%
%    o cache_view: the cache view.
%
*/
MagickExport IndexPacket *GetCacheViewIndexes(ViewInfo *cache_view)
{
  ...
  const unsigned int size_to_fit,RectangleInfo *region_info)
{
  ...
  if (size_to_fit != MagickFalse)
    return((int) ParseSizeGeometry(image,geometry,region_info,&image->exception));
  ...
  const unsigned char *source)
{
  ...
  if (quantum_info == (QuantumInfo *) NULL)
    return(0);
  length=ImportQuantumPixels(image,(ViewInfo *) NULL,quantum_info,quantum,
            ...
            if (order == 1)
              {
                if (intensity < lower_threshold)
                  ...
                  threshold=(MagickRealType) (QuantumRange*GetPseudoRandomValue());
              }
            else if (order == 2)
...
%
%      MagickBooleanType SyncCacheViewPixels(ViewInfo *cache_view)
%
%  A description of each parameter follows:
%
%    o cache_view: the cache view.
...
%
*/
MagickExport MagickBooleanType SyncCacheViewPixels(ViewInfo *cache_view)
{
  MagickBooleanType
    status;
