  ...
  if (*image_info->filename == '@')
    {
      ...
      /*
        A file with a list of image filenames to read.
      */
      command=FileToString(image_info->filename+1,~0,exception);
      if (command == (char *) NULL)
        return((Image *) NULL);
      StripString(command);
      image_list=StringToArgv(command,&number_images);
      command=DestroyString(command);
      read_info=CloneImageInfo(image_info);
      images=NewImageList();
      for (i=1; i < number_images; i++)
      {
        (void) CopyMagickString(read_info->filename,image_list[i],
          MaxTextExtent);
      ...
      for (i=0; i < number_images; i++)
        image_list[i]=DestroyString(image_list[i]);
      image_list=(char **) RelinquishMagickMemory(image_list);
      return(images);
    }
  (void) InterpretImageFilename(filename,MaxTextExtent,image_info->filename,
    (int) image_info->scene);
  if (LocaleCompare(filename,image_info->filename) != 0)
    {
      ...
      */
      ...
      {
        (void) InterpretImageFilename(read_info->filename,MaxTextExtent,
          filename,(int) scene);
        image=PingImage(read_info,exception);
        if (image == (Image *) NULL)
          continue;
  ...
  */
  ...
  for (next=image; next != (Image *) NULL; next=GetNextImageInList(next))
  {
    ...
    (void) FormatMagickTime(GetBlobProperties(next)->st_mtime,MaxTextExtent,
      timestamp);
    (void) SetImageProperty(next,"modify-date",timestamp);
    (void) FormatMagickTime(GetBlobProperties(next)->st_ctime,MaxTextExtent,
      timestamp);
    (void) SetImageProperty(next,"create-date",timestamp);
  ...
  if (LocaleCompare(filename,image_info->filename) != 0)
    {
      ...
      */
      ...
      images=NewImageList();
      extent=(long) (read_info->scene+read_info->number_scenes);
      for (scene=read_info->scene; scene < (long) extent; scene++)
      {
        (void) InterpretImageFilename(read_info->filename,MaxTextExtent,
          filename,(int) scene);
        image=ReadImage(read_info,exception);
