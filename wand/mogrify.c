        ...
        if (LocaleCompare("adaptive-resize",option+1) == 0)
          {
            ...
            */
            (void) SyncImageSettings(image_info,*image);
            (void) ParseSizeGeometry(*image,argv[i+1],&geometry,exception);
        ...
        if (LocaleCompare("geometry",option+1) == 0)
          {
            ...
            flags=ParseSizeGeometry(*image,argv[i+1],&geometry,exception);
        ...
        if (LocaleCompare("level-colors",option+1) == 0)
          {
            ...
            if ( isalpha((int) token[0]) || token[0] == '#' )
              GetMagickToken(p,&p,token);
            /* special case, no divider, just a single color */
            if ( token[0] == '\0' )
        ...
        if (LocaleCompare("sparse-color",option+1) == 0)
          {
            ...
            if (i == (long) (argc-1))
              ThrowMogrifyException(OptionError,"MissingArgument",option);
/*  This does not work if first number is a '%' escape!
            (void) strtod(argv[i],&p);
            if (p == argv[i])
              ThrowMogrifyInvalidArgumentException(option,argv[i]);
*/
  ...
  Image *image)
{
  ...
  */
  ...
  if (option != (const char *) NULL)
    (void) QueryColorDatabase(option,&image->border_color,
      &image->exception);
  ...
  if (option != (const char *) NULL)
    image->interlace=(InterlaceType) ParseMagickOption(
      MagickInterlaceOptions,MagickFalse,option);
  ...
  if (option != (const char *) NULL)
    (void) QueryColorDatabase(option,&image->matte_color,
      &image->exception);
